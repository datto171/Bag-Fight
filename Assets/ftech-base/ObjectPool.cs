...
{
    /// <summary>
    /// it is base class of object pooling.
    /// -- CreatePool(int size) - create new pool and spawn original object based on size.
    /// -- Spawn() - get object from pool or spawn new object if pool is empty.
    /// -- Recycle() - return object to the pool or destroy object if this object isn't a child of pool.
    /// -- CountPooled() - return total objects are into the pool now.
    /// -- CountSpawned() - return total objects are into the scene now.
    /// -- GetSpawned() - return list of objects are into the scene now.
    /// -- GetPooled() - return list of objects are into the pool now.
    /// -- DestroyPooled() - destroy all objects are into the pool now.
    /// -- DestroyAll() - Recycle all objects are into the scene now and then destroy all pool.
    /// </summary>
    public sealed class ObjectPool : MonoBehaviour
    {
        ...
        public static void Recycle(GameObject obj, GameObject prefab)
        {
            ...
            {
                Debug.Log("Something wrong when recycle object, perhaps object is already recycled or has been destroy");
            }
        }
        ...
        {
            instance.pooledObjects.Keys.ToList().ForEach(g => DestroyAll(g));
        }
